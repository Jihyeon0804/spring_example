...
<body>
	<div class="container">
		...
		<%--<form method="post" action="/lesson06/ex01/add-user"> --%>
			...
			<%-- ★★★★★★★ AJAX 통신일 때는 반드시 ★button 타입★으로 지정한다 ★★★★★★★ --%>
...
<script>
	$(document).ready(function() {
		...
		$('#joinBtn').on('click', function() {
			...
			$.ajax({
				// request
				// type 생략 가능(default : get)
				type:"POST"
				
				// form 태그의 action과 동일
				, url:"/lesson06/ex01/add-user"
				
				// data는 JSON String으로 (input의 name 속성이 key명, 값을 꺼내서 저장한 변수명이 value)
				, data:{"name":name, "yyyymmdd":yyyymmdd, "email":email, "introduce":introduce}
					
				// resonse
				// call back 함수
				// success - 에러가 나지 않았을 때, complete - 결과에 상관없이 무조건 불려짐, error - 에러 발생했을 때
				, success:function(data) {	// data: response 결과 응답값(String) (!request의 data 아님!)
